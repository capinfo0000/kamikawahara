...
import javax.servlet.http.HttpServletResponse;
	...
	public class DetailServlet extends HttpServlet {
	    ...
	    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
	    	...
	    	for(int i = 0; i < rowCount; i++) {
	    		...
	    		if (!cAmt.isEmpty()) {
	    			if(cAmt.matches("^[0-9,]+$")) {
	    				...
	    				errorMessages.add("貸方金額に半角数字以外が含まれています。");
	    				hasFormatError = true;
	    			}
	    		}
	    		...
	    		//貸借金額一致チェック（数字エラーがない場合のみ実行）
	    		if (errorMessages.isEmpty()) {
	    			if(debitTotal != creditTotal) {
	    				errorMessages.add("合計金額が一致しません");
	    			}
	    		}
	    		
	    		if (!errorMessages.isEmpty()) {
	    			//エラーメッセージと入力されていた各種データをリクエストにセット
	    	...
	    	System.out.println("合計金額：￥" + String.format("%,d", debitTotal));
	    	System.out.println("備考 : " + note);
	    	
	    	//閲覧モードに移行
	    	response.sendRedirect(request.getContextPath() + "/detail?mode=view");
